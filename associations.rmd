...
genera <- genera[genera != "uncultured"]
```

Let's visuzlize the effect sizes and significant tests.

```{r, fig.width=9, fig.height=3}
ggplot(tests[padj < 0.05 & genus %chin% genera],
...
ggsave(sprintf("figures/lfcs.svg"), width = 9, height = 3)
```
...
genera <- tests[padj < 0.05 & genus %chin% genera, unique(genus)]
counts <- counts[taxa %in% genera & compartment %in% c("biofilter", "root")]
map <- rep(c("commercial", "established"), each = 3)
...
counts[, inoculum := factor(inoculum, levels = c("commercial", "established"))]
