...
for (s in subsets) {
    ...
    res <- association(psub, vars, min_count = 100, in_samples=0.05)
    res[, "site" := s[1]]
    tests[[s[1]]] <- res
}
...
```{r}
ggplot(tests, aes(x=pvalue)) + geom_histogram(bins=20, alpha=0.5) + geom_histogram(aes(x=padj), bins=20, fill="blue", alpha=0.5)
```
...
sigs <- roots[padj < 0.2 & variable == "inoculate", unique(genus)]
data <- plot_counts(psub, "inoculate", taxa=sigs, only_data=T)
inoc_plot %+% data
