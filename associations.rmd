...
for (s in subsets) {
    ...
    res <- association(psub, vars, in_samples=0.5)
    res[, "site" := s[1]]
    tests[[s[1]]] <- res
}
...
#pval_model <- ihw(pvalue ~ baseMean, 0.05, data=tests)
#tests[, padj := adj_pvalues(pval_model)]

ggplot(tests, aes(x=pvalue)) + geom_histogram(bins=20) + geom_histogram(aes(x=padj), bins=20, fill="blue")
```

Let's visuzlize the effect sizes and significant tests.

```{r, fig.width=10, fig.height=7}
lfc_plot <- ggplot(tests[padj < 0.05],
...
sigs <- bioballs[padj < 0.05 & variable == "inoculate", unique(genus)]
...
sigs <- bioballs[padj < 0.05 & variable == "height", unique(genus)]
...
```{r, fig.height=9, fig.width=9}
sigs <- bioballs[padj < 0.05 & variable == "no_leaves", unique(genus)]
...
sigs <- bioballs[padj < 0.05 & variable == "root_length", unique(genus)]
...
sigs <- roots[padj < 0.05 & variable == "inoculate", unique(genus)]
data <- plot_counts(psub, "inoculate", taxa=sigs, only_data=T)
inoc_plot %+% data
...
sigs <- roots[padj < 0.05 & variable == "height", unique(genus)]
plot_counts(psub, variable="height", taxa=sigs) +
    geom_smooth(method="glm") + theme_bw()
...
sigs <- roots[padj < 0.05 & variable == "no_leaves", unique(genus)]
plot_counts(psub, variable="no_leaves", taxa=sigs) +
    geom_smooth(method="glm") + theme_bw()
...
```{r, fig.height=3, fig.width=5}
sigs <- roots[padj < 0.05 & variable == "root_length", unique(genus)]
plot_counts(psub, variable="root_length", taxa=sigs) +
    geom_smooth(method="glm") + theme_bw()
