...
```{r}
ggplot(plants[sample == "Growth Bed" & !is.na(height_avg)], aes(x=date, y=height_avg, col=inoculum)) + 
  geom_point() + stat_summary(aes(group=inoculum), fun.y=mean, geom="line") + theme_bw() + 
...
ggplot(plants[sample == "Growth Bed" & !is.na(root_length_avg)], aes(x=inoculum, y=root_length_avg, col=inoculum)) +
  geom_boxplot() + geom_jitter(width=0.2) + theme_bw() +
  labs(y="average root length [cm]", color="tank", x="", color="") + guides(color=FALSE) +
ggsave("figures/root_length_avg.pdf")
```